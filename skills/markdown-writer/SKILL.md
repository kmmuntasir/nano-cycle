...
**Chronic LLM failure:** metadata header blocks. Stacked
`**Label:** value` lines —
...
— render as ONE fused paragraph. Every line that must stay separate needs
two trailing spaces. The validator hard-errors on this pattern; do not ship
a stacked label block without them.
...
### 8. Mermaid diagram syntax

Mermaid's lexer fails hard on shape-delimiter mismatches, and the failure is
invisible until render. Rules:

- **Quote every node and edge label**: `A["Deploy gate"]`, `B -- "ship all" --> C`.
  Quoted contents are immune to every bracket, paren, and pipe pitfall.
- **Pair shape delimiters exactly** — an opener must close with its matching
  token: `[/text/]` (parallelogram), `[\\text\\]`, `[[text]]`, `([text])`,
  `[(text)]`, `((text))`, `{{text}}`. Writing `A[/text]` (opener `[/`, closer
  `]`) is a fatal lexical error.
- The first non-empty line of the block must declare the diagram type
  (`flowchart TD`, `sequenceDiagram`, …).
- No `<br/>` or HTML in labels unless necessary — prefer plain text lines.
...
```bash
node <this-skill-directory>/validate_md.cjs <path-to-file.md>
# <this-skill-directory> = the directory containing this SKILL.md (shown in
# the skill listing). The validator sits right next to it.

(The validator is CommonJS; `.cjs` extension keeps it runnable when the repo root
sets `"type": "module"`.)
```
...
- Unclosed fences.
- Stacked bold-label lines (`**Label:** value` blocks) missing hard breaks.
- Mermaid blocks: known diagram-type declaration, shape-delimiter pairing
  (`[/` with `/]`, `[[` with `]]`, …), balanced brackets and quotes per line.
- Every mermaid block is additionally **rendered** with mermaid-cli. If
  `mmdc` is not on PATH, the validator auto-installs it into a per-user cache
  dir (`~/.cache/md-validator` on Linux, `~/Library/Caches` on macOS,
  `%LOCALAPPDATA%\md-validator` on Windows) — no sudo, no global state; the
  first install downloads a headless browser (~2-5 min), later runs reuse it.
  Opt out with `MD_VALIDATE_NO_INSTALL=1` (the static lint still runs).
  Render failures are errors; a browser-launch/download failure is a warning.
- Warning: sentence-style lines that look like intended hard breaks missing
  trailing two spaces (only under `--strict`).

### Limitations

The validator does NOT check: list continuation indentation (4-space rule),
angle-bracket swallowing, or underscore italics. Apply rules 4, 5, and 7 by
hand during the write phase. Re-read the rendered file once before declaring
done.
