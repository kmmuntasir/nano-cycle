...
- ✅ **V12** rounds exhausted (owner run `20260921-011634429`) — deps-declared failed at round 0, no fix rounds allowed, verify failed, branch kept. Also exposed an engine bug: the failure early-return skipped persisting the terminal state (disk stayed "running") — fixed with a single settle() path + a persistence assertion in the harness.
...
## V12 — fix rounds exhausted → honest failure ✅ (owner run `20260921-011634429` — and it caught a real engine bug: the failure path skipped state persistence; fixed + regression-tested)
